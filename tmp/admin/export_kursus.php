<?php
if (!empty($_GET['jenis'])) { $jenis = $_GET['jenis']; }
else { $jenis = "ALL"; }
...
if ($jenis == 1) {
	$output = "Bil\tNama\tNo. Kad Pengenalan\tBahagian\tKumpulan Perkhidmatan\tKursus/Latihan\t\n";
}
elseif ($jenis == 2) {
	$output = "Bil\tNama\tNo. Kad Pengenalan\tBahagian\tKumpulan Perkhidmatan\tSesi Pembelajaran\t\n";
}
elseif ($jenis == 3) {
	$output = "Bil\tNama\tNo. Kad Pengenalan\tBahagian\tKumpulan Perkhidmatan\tPembelajaran Kendiri\t\n";
}
else {
...
while ($t = mysql_fetch_array($d)) {
	...
	if ($jenis == "ALL") {
		...
		if ($zaki) {
			$n = $n + 1;
			if ($jenis == 1) {
				if ($kursus >= 1) { $display = ($kursus / 6) . " HARI"; }
				else { $display = "0"; }
			}
			elseif ($jenis == 2) {
				$hari = floor($sesi / 6);
			...
			$dtotal = str_replace(" 0 JAM","",str_replace(".5 JAM"," JAM 30 MINIT",$sesid));
			$output .= $n . "\t" . output1($t['unama']) . "\t" . output1($t['unokp']) . "\t" . output1($t['bahagian']) . "\t" . output2($t['kumpulan']) . "\t" . $display . "\t\n";
		}
	}
}
header('Content-type: application/ms-excel');
header('Content-Disposition: attachment; filename=LaporanKursus-'.date("Ymd_hia").'.xls');
print $output;
?>